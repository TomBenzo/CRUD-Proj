...
def token_required(func):
    ...
    def decorated(*args,**kwargs):
        ...
        if 'x-access-token' in request.headers:
            ...
            return {
                'status':'not ok',
                'message':"Missing Auth Header. Please add the header 'x-access-token' to your Headers."
            }
        if not token:
            return {
                'status':'not ok',
                'message':"Missing Auth Token. Please log into a user that has a valid token."
            }
